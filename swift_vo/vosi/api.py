...
async def obsloctap_availability() -> Response:
    """
    VOSI-availability endpoint for ObsLocTAP.

    Returns the availability status of the service as required by DALI and ObsLocTAP specifications.
    The endpoint follows IVOA VOSI (Virtual Observatory Support Interfaces) standard.
    """
    xml_content = Availability(available=True, up_since=SERVICE_STARTUP_TIME).to_xml(
        encoding="UTF-8", xml_declaration=True
...
async def obsloctap_capabilities():
    """Returns the VOSI capabilities document for ObsLocTAP service."""
    base_url = f"https://{VO_SERVER}{VO_ROOT_PATH}"
    ...
    capabilities = VOSICapabilities(
        capability=[
            Capability(
                ...
                interface=[
                    Interface(
                        access_url=[AccessURL(value=f"{base_url}/obsloctap/capabilities", use="full")],
                        use="base",
                        version="1.0",
                    )
                ],
            ...
            Capability(
                ...
                interface=[
                    Interface(
                        access_url=[AccessURL(value=f"{base_url}/obsloctap/availability", use="full")],
                        version="1.0",
                    )
                ],
            ),
            Capability(
                standard_id="ivo://ivoa.net/std/ObsLocTAP#query-1.0",
                interface=[
                    Interface(
                        access_url=[AccessURL(value=f"{base_url}/obsloctap/query", use="full")],
                        role="std",
                        version="1.0",
                    )
                ],
